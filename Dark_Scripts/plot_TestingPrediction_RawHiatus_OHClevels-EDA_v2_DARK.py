...
label = r'\textbf{SLOWDOWN ONSET [ Normalized ]}'
...
for ppp in range(ohcHIATUS.shape[0]):
    ...
    if ppp == 1:
        lrpz = lrp/np.max(lrp)
        csc = m.contour(x,y,lrpz,np.arange(0.2,1.1,0.2),linestyles='-',latlon=True,
                        colors='gold',linewidths=0.5)
...
if rm_ensemble_mean == True:
    plt.savefig(directoryfigure + 'RawCompositesHiatus_OHClevels_v2_AccH-%s_AccR-%s_rmENSEMBLEmean_DARK.png' % (accurate,accurate),dpi=300)
else:
    plt.savefig(directoryfigure + 'RawCompositesHiatus_OHClevels_v2_AccH-%s_AccR-%s_DARK.png' % (accurate,accurate),dpi=300)