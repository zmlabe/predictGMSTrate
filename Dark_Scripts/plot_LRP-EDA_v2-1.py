...
cs1 = m.contourf(x,y,var_Robs,limit,extend='max',latlon=True)
cs1.set_cmap(cmap) 
...
cs2 = m.contourf(x,y,var_Hobs,limit,extend='max',latlon=True)
cs2.set_cmap(cmap) 
...
if rm_ensemble_mean == True:
    plt.savefig(directoryfigure + 'LRP-EDA_Observations_v2-1_AccH-%s_AccR-%s_rmENSEMBLEmean.png' % (accurateH,accurateR),dpi=300)
else:
    plt.savefig(directoryfigure + 'LRP-EDA_Observations_v2-1_AccH-%s_AccR-%s.png' % (accurateH,accurateR),dpi=300)