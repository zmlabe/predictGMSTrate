...
### Read in PDO index
PDO = np.genfromtxt(directorydata + '/PDO/PDO_CESM2LE_1990-2099.txt',unpack=True)
PDOtest = PDO.transpose()[testindices,:yearsall.shape[0]]
...
cs = plt.pcolormesh(PDOtest,shading='faceted',edgecolor='w',
...
for i in range(act_nan_re.shape[0]):
    for j in range(act_nan_re.shape[1]):
        if pre_nan_re[i,j] == 1:
            ...
            predictedsavePDOall.append(PDOtest[i,j])
        if np.isnan(act_nan_re[i,j]) == False:
            if pre_nan_re[i,j] == 1:
                cc ='gold'
                predictedsavePDO.append(PDOtest[i,j])
            else:
                cc = 'k'        
            savePDO.append(PDOtest[i,j])
            plt.text(j+0.56,i+0.48,r'\textbf{H}',fontsize=9,
                color=cc,va='center',ha='center')
savePDO = np.asarray(savePDO)
meanPDOsave = np.nanmean(savePDO)
predictedsavePDO = np.asarray(predictedsavePDO)
meanPDOpredictedsave = np.nanmean(predictedsavePDO)
predictedsavePDOall = np.asarray(predictedsavePDOall)
meanPDOpredictedsaveall = np.nanmean(predictedsavePDOall)
...
cbar.set_label(r'\textbf{ANNUAL PDO INDEX [CESM2-LE]}',
                color='k',labelpad=10,fontsize=23)

plt.ylabel(r'\textbf{Ensemble Member}',color='k',fontsize=12)

plt.tight_layout()
plt.savefig(directoryfigure + 'PDO_heatmap_hiatus_v2.png',dpi=300)