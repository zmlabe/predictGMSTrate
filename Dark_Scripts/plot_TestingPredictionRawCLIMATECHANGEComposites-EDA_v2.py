"""
Explore raw composites based on indices from predicted testing data for
only the climate change events
...
### Pick all climate change
if accurate == True: ### correct predictions for climate change
    ohc_allenscomp = []
    for ens in range(ohcready.shape[0]):
        ohc_comp = []
        for yr in range(ohcready.shape[1]):
            if (pre_re[ens,yr]) == 0 and (act_re[ens,yr] == 0):
                ohc_comp.append(ohcready[ens,yr,:,:])
        ohc_allenscomp.append(ohc_comp)
elif accurate == False: ### picks all climate change predictions
    ohc_allenscomp = []
    for ens in range(ohcready.shape[0]):
        ohc_comp = []
        for yr in range(ohcready.shape[1]):
            if pre_re[ens,yr] == 0:
                ohc_comp.append(ohcready[ens,yr,:,:])
        ohc_allenscomp.append(ohc_comp)
elif accurate == 'WRONG': ### picks climate change but is wrong
    ohc_allenscomp = []
    for ens in range(ohcready.shape[0]):
        ohc_comp = []
        for yr in range(ohcready.shape[1]):
            if (pre_re[ens,yr]) == 0 and (act_re[ens,yr] == 1):
                ohc_comp.append(ohcready[ens,yr,:,:])
        ohc_allenscomp.append(ohc_comp)
elif accurate == 'CLIMATECHANGE': ### actual climate change
    ohc_allenscomp = []
    for ens in range(ohcready.shape[0]):
        ohc_comp = []
        for yr in range(ohcready.shape[1]):
...
### Composite climate change for 8 ensembles
...
### Composite across all ensembles to get climate change
...
label = r'\textbf{%s - [ CLIMATE CHANGE COMPOSITE ]}' % vari_predict[0]
...
plt.tight_layout()
if rm_ensemble_mean == True:
    plt.savefig(directoryfigure + 'RawCompositesHiatusCLIMATECHANGE_v2_AccH-%s_AccR-%s_rmENSEMBLEmean.png' % (accurate,accurate),dpi=300)
else:
    plt.savefig(directoryfigure + 'RawCompositesHiatusCLIMATECHANGE_v2_AccH-%s_AccR-%s.png' % (accurate,accurate),dpi=300)
...
def netcdfLRP(lats,lons,var,directory,typemodel,saveData):
    ...
    name = 'OHC_comp/OHCcomp_' + typemodel + '_' + saveData + '.nc'
...
netcdfLRP(lats,lons,ohcHIATUS,directorydata,'climatechange_True',saveData)