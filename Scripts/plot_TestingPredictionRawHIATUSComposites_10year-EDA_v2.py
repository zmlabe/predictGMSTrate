    ...
    for ens in range(ohcready.shape[0]):
        ...
        for yr in range(ohcready.shape[1]):
            if (pre_re[ens,yr]) == 1 and (act_re[ens,yr] == 1):
                ohc_comp.append(np.nanmean(ohcready[ens,yr:yr+10,:,:],axis=0))
...
plt.tight_layout()
if rm_ensemble_mean == True:
    plt.savefig(directoryfigure + 'RawCompositesHiatus_10year_v2_AccH-%s_AccR-%s_rmENSEMBLEmean.png' % (accurate,accurate),dpi=300)
else:
    plt.savefig(directoryfigure + 'RawCompositesHiatus_10year_v2_AccH-%s_AccR-%s.png' % (accurate,accurate),dpi=300)