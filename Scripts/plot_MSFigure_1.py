...
leg = plt.legend(shadow=False,fontsize=8,loc='upper center',
                 bbox_to_anchor=(0.5,1.15),fancybox=True,ncol=2,frameon=False,
...
plt.xlabel(r'\textbf{Number of slowdown events per ensemble member}',fontsize=10,color='k')
plt.yticks(np.arange(0,1.1,0.1),map(str,np.round(np.arange(0,1.1,0.1),2)),size=9)
plt.xticks(np.arange(0,21,2),map(str,np.round(np.arange(0,21,2),2)),size=9)
plt.xlim([-0.5,20])   
plt.ylim([0,0.4])
