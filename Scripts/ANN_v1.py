...
for i in range(len(modelGCMs)):
    ...
    modelsq_var,lats,lons = read_primary_dataset(vari_predict[0],dataset,monthlychoice,numOfEns,
                                            ...
                                            ravelbinary,shuffletype,timeper,
                                            lat_bounds,lon_bounds)
    obsq_var,lats,lons = read_obs_dataset(vari_predict[0],dataset_obs,numOfEns,lensalso,randomalso,ravelyearsbinary,ravelbinary,shuffletype,lat_bounds=lat_bounds,lon_bounds=lon_bounds)
...
def loadmodel(Xtrain,Xtest,Ytrain,Ytest,hidden,random_network_seed,n_epochs,batch_size,lr_here,ridgePenalty,actFun,input_shape,output_shape):
...
model,history = loadmodel(XtrainS,XtestS,Ytrain,Ytest,hidden,random_network_seed,n_epochs,batch_size,lr_here,ridgePenalty,actFun,input_shape,output_shape)
