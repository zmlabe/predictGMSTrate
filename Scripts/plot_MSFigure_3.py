...
limitd = np.arange(-0.2,0.21,0.01)
barlimd= np.round(np.arange(-0.2,0.21,0.1),2)
cmapd = cmr.cm.redshift
labeld = r'\textbf{Difference [Relevance]}'
...
### Variable
# DIFF_hiatus = hiatus_correctz - hiatus_wrongz
DIFF_hiatus = hiatus_correct - hiatus_wrong
hiatus_wrongz = DIFF_hiatus/np.max(DIFF_hiatus)

circle = m.drawmapboundary(fill_color='dimgrey',color='dimgray',
                  linewidth=0.7)
circle.set_clip_on(False)

cs4 = m.contourf(x,y,DIFF_hiatus,limitd,extend='both',latlon=True)
cs4.set_cmap(cmapd) 

m.fillcontinents(color='dimgrey',lake_color='dimgrey')
plt.title(r'\textbf{[a] MINUS [c]}',fontsize=13,color='dimgrey')         
