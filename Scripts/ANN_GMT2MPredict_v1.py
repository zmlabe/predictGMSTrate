"""
Create an ANN to predict hiatus from only the GMT2M
...
### Standardize
# XtrainS,XtestS,XvalS,stdVals = dSS.standardize_dataVal(T2Mtrain,T2Mtest,T2Mval)
# XtrainS = XtrainS.reshape(trainindices.shape[0]*yearsall.shape[0],1)
# XvalS = XtestS.reshape(valindices.shape[0]*yearsall.shape[0],1)
# XtestS = XvalS.reshape(testindices.shape[0]*yearsall.shape[0],1)
Xtrain = T2Mtrain.reshape(trainindices.shape[0]*yearsall.shape[0],1)
Xval = T2Mtest.reshape(valindices.shape[0]*yearsall.shape[0],1)
Xtest = T2Mval.reshape(testindices.shape[0]*yearsall.shape[0],1)
