...
counter = 0
for yri in range(classes_mhplot.shape[1]):
    exampleclass = classes_mhplot[exampleens,yri]
    if np.isfinite(exampleclass) == True:
        counter+=1
        if counter == 1:
            plt.axvline(yearsallhiatus[yri],ymin=0,ymax=0.7,linestyle='--',color='crimson',linewidth=2,
                        dashes=(1,0.3),zorder=1,label=r'\textbf{SLOWDOWN EVENT}',
                        alpha=0.5)
        else:
            plt.axvline(yearsallhiatus[yri],ymin=0,ymax=0.7,linestyle='--',color='crimson',linewidth=2,
                        dashes=(1,0.3),zorder=1,alpha=0.5)

ax.fill_between(yearsall[11:112],minens[11:112],
                maxens[11:112],facecolor='dimgrey',
                alpha=0.25,zorder=2,clip_on=False)

plt.plot(yearsall,modelsm.squeeze()[exampleens],color='teal',linewidth=3,
         alpha=1)
...
plt.text(2030,3.2,r'\textbf{CESM2}',fontsize=20,color='dimgrey')
plt.text(2033.3,2.8,r'\textbf{ERA5}',fontsize=20,color='k')
...
for yri in range(classes_mhplot.shape[1]):
    exampleclass = classes_mhplot[exampleens,yri]
    if np.isfinite(exampleclass) == True:
        plt.axvline(yri,linestyle='--',color='crimson',linewidth=2,
                    dashes=(1,0.3),zorder=1,alpha=0.5)
...
plt.text(101,(SLOPEthreshh_m*diff_o)[-1],r'\textbf{CESM2-LE Threshold}',color='crimson',fontsize=9,
         va='center')
plt.text(101,timetrend_m[exampleens,-1],r'\textbf{Ensemble \#%s}' % (actualnumberENS),color='teal',fontsize=9,
         va='center')
...
ax.tick_params('both',length=3.5,width=1,which='major',color='dimgrey')  
...
for i in range(len(patches_count_pre)):
    patches_count_pre[i].set_facecolor('darkslategray')
...
plt.ylabel(r'\textbf{Frequency}',fontsize=7,color='k')
plt.xlabel(r'\textbf{Number of slowdown events per member}',fontsize=7,color='k')
...
plt.savefig(directoryfigure + 'Figure_1.png',dpi=600)