...
directorymodel = '/Users/zlabe/Documents/Research/GmstTrendPrediction/SavedModels/HiatusConstantDefinition/'
...
directoryfigure = '/Users/zlabe/Desktop/GmstTrendPrediction/ANN_v2/LRP/HiatusConstantDefinition/'
...
filenametest = directorydata  + 'ConstantDefinition_LRPMap' + nametypetest + '_' + savename + '.nc'
...
nametypeobs = 'Obs'
filenameobs = directorydata  + 'ConstantDefinition_LRPMap' + nametypeobs + '_' + savename + '.nc'
...
label = r'\textbf{LRP Relevance - ANN using constant definition}'
...
plt.title(r'\textbf{\textit{NO} SLOWDOWN}',fontsize=17,color='dimgrey')
...
plt.title(r'\textbf{SLOWDOWN}',fontsize=17,color='dimgrey')
...
plt.tight_layout()
if rm_ensemble_mean == True:
    plt.savefig(directoryfigure + 'HiatusConstantDefinition_LRP-EDA_Testing_v2_AccH-%s_AccR-%s_rmENSEMBLEmean.png' % (accurateH,accurateR),dpi=300)
else:
    plt.savefig(directoryfigure + 'HiatusConstantDefinition_LRP-EDA_Testing_v2_AccH-%s_AccR-%s.png' % (accurateH,accurateR),dpi=300)
...
if rm_ensemble_mean == True:
    plt.savefig(directoryfigure + 'HiatusConstantDefinition_LRP-EDA_Observations_v2_AccH-%s_AccR-%s_rmENSEMBLEmean.png' % (accurateH,accurateR),dpi=300)
else:
    plt.savefig(directoryfigure + 'HiatusConstantDefinition_LRP-EDA_Observations_v2_AccH-%s_AccR-%s.png' % (accurateH,accurateR),dpi=300)
...
def netcdfLRP(lats,lons,var,directory,typemodel,saveData):
    ...
    name = 'LRP_comp/HiatusConstantDefinition_LRPMap_comp_' + typemodel + '_' + saveData + '.nc'
...
netcdfLRP(lat,lon,hiatuslrpm,directorydata,'actualhiatus',savename)
netcdfLRP(lat,lon,hiatuslrpmobs,directorydata,'actualhiatus_obs',savename)